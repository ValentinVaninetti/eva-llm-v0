...
**How corrections are handled here.** Superseded sections are kept as written, with a correction banner at the top pointing to the section that replaces them, because the sequence of what was believed and on what evidence is part of the record. As of 2026-08-25 there are four such banners: Section 2 (`--seed` never varied initialization), Section 4.7 (two incompatible band definitions were compared), Section 4.8 (a surprisal was read with the wrong sign, inverting a real-text result from positive to null), and Sections 4.10–4.11 (measurements stand; the capacity explanation offered for them was refuted). Two of the four were sign or definition errors rather than measurement errors, and both survived review for days — see Limitations.
...
Applied to real text (a full novel, *Don Quijote*), the temperature fix shows no in-distribution benefit: global bpb is null, and it is also null on a targeted entity-retrieval metric built to isolate long-range recall from general language modeling. (That second metric is *not* null for ClockMem's memory itself — see the end of this abstract; it is null for the *temperature fix* specifically, which is the claim being made here.) Its one positive real-text result is narrower than hoped: under state carried across windows the model was never trained for (an out-of-distribution regime), the fixed model remains stable while the unfixed one collapses — reproduced independently on two machines in a full cross-validated control. A follow-up baseline (a fixed, non-learned slow decay rate on every channel, built inside this same codebase rather than porting an external SSM) shows the same carry-stability, ruling out that it is specific to the learned-and-compressed mechanism of the temperature fix — any sufficiently slow recurrence gets it for free. In no condition tested, including this baseline, does carried state at long range ever outperform the same model's own no-memory reference: stability is not usefulness, in either mechanism.

Those real-text nulls — as they were understood at the time, before the sign error was found — motivated a reframing: the synthetic benchmark used throughout Section 4 tests purely *positional* recall (a fixed, globally-known lag), not the *content-addressed* recall that entity recognition in real text actually requires. A new synthetic benchmark built for content-addressed recall (in-context key→value binding at random, unknown-in-advance distances — the standard associative-recall/induction-head task family) settles two things. With enough *unique* training data, ClockMem solves it: 99.85% held-out accuracy at short range and still 97.7% at 256 bytes, with both no-leakage controls holding — an outcome that contradicts a design claim asserted in this codebase's own source comments, and that a smaller-data version of the same experiment had badly underestimated. A full-attention baseline on the identical task reaches only ~47%, flat across distance. Classifying what each model actually predicts turns that number into a mechanism: attention places 89.7% of its predictions on one of the two values genuinely bound in that window — it locates the candidate set nearly perfectly — but then splits them 47/42, a coin flip between correct candidates. It retrieves without binding. ClockMem at 2 keys, by contrast, confuses the two keys in exactly 0.00% of 5,967 queries. The capability does not scale: at 8 keys, matched for data and distance, ClockMem drops to 31.8% and falls into attention's failure mode — it identifies the candidate set even better (99.91%) while picking the wrong key's value 76.5% of the time. The 8-key failure is therefore specifically a *binding* failure, not degraded memory or an unlearned task.

**We first attributed that collapse to a capacity limit — a partition of channels running out of room — and that was wrong.** Decoupling width from key count shows two configurations with identical channels-per-key differing by 21 points; a query-swap diagnostic shows that several configurations never read the query at all, moving their output *less* when asked about a different key than when a filler byte is flipped; ablation localizes the entire binding capability of a 5.4M-parameter model to **4 channels out of 256**, and shows that the models which fail use those same four channels for a weaker computation. Most decisively, holding weights and corpus fixed and varying only the order in which the same windows are visited, the binding behaviour appears in **2 runs out of 17 (11.8%)** — and when it appears it reaches 99.77%. The element-wise write *can* represent a binding; gradient descent almost never finds it. **The problem is optimization, not representation**, which withdraws the stated motivation for an outer-product write.

Finally, none of this transfers. Eight seeds on the full novel show no lottery in real text: global bpb is unimodal (mean 1.7235, range 0.027) and per-seed entity retrieval sits within 1.4 standard deviations, against ~13 standard deviations separating winner from blind in the synthetic task. The lottery is a property of the toy. The same eight seeds, however, deliver the draft's most robust real-text result, and one we were not looking for: **ClockMem's in-context memory measurably helps between roughly 32 and 256 bytes**, consistently across all eight, switching off beyond 256 — a finding that had been reported as null in an earlier section because of a sign error in the evaluation tool, now corrected.
...
> **RETRACTED (2026-08-25): the "partially-distributed spectrum" reported below was an artifact of comparing two different band definitions.** The repository contains two, and they are not compatible:
>
> | source | "fast" | "medium" | "slow" |
> |---|---|---|---|
> | training trace (`src/train.rs:59`) | `α < 0.05` | `0.05 ≤ α < 0.3` | `α ≥ 0.3` |
> | `examples/alpha_dispersion.rs` | `α < 0.9` (mem < 10 tok) | `α < 0.99` (10-100) | `α < 0.999` (100-1000) |
>
> Effective memory is ≈ `1/(1-α)`, so the trace's "slow" threshold of `α ≥ 0.3` is **1.4 tokens** of memory — not slow by any reading. Section 3's spectra are in *memory-length* bands; the Quijote row below was read off the *training trace*. Measured consistently, in memory-length bands, the Quijote spectrum is **501 fast (98%) / 10 medium / 1 very slow at `dim=512`** — the same collapse as everywhere else, not an exception to it. The claim that a large narrative corpus induces a distributed spectrum on its own is **withdrawn**; nothing about Section 3's saturation trap is corpus-dependent. The table is kept below as originally written, with its units now labelled.

**An observation flagged alongside the null result — see the retraction above before reading it.** In *training-trace bands* (`α<0.05 / <0.3 / ≥0.3`, which are NOT memory-length bands), the `T=1` spectrum on this corpus looks less collapsed than elsewhere. At steady state (step 3500):

| | `α<0.05` | `0.05≤α<0.3` | `α≥0.3` |
|---|---|---|---|
| T=1 on Quijote (trace bands) | ≈34% | ≈39% | ≈26% |
| T=1.3 on Quijote (trace bands) | ≈16% | ≈54% | ≈30% |
| T=1 on the 250-411KB technical-prose corpora (Section 3.1) — **memory-length bands, not comparable to the rows above** | 98% | 2% | ~0% |

Under the memory-length definition all three rows are ≈98% fast. Neither `T=1` nor `T=1.3` translates into better bpb here, and two explanations remain live and undistinguished: (a) `seq=512` may still be short relative to a novel's actual dependency distances (a name or plot thread returning hundreds of pages later is far more than 512 bytes away), or (b) one epoch on a 13M-parameter model may be too little training for any available long-range memory to get exploited.
...
> **SIGN ERROR, CORRECTED 2026-08-25 — this section originally reported the opposite of what its own data says.** The `Δlnp` column is built from `mean_lnp`, which despite its name is a **surprisal** (`-log p`; the tool computes `-((row[b]-m) - ln Σexp)`), so **lower is better and a NEGATIVE Δlnp means memory HELPED**. The tool's header comment asserted the reverse, and the paragraph below inherited it, converting a positive result into a reported null. The sign is settled by two quantities in the same table that do not depend on it — `top1%` and `rank` — and both agree with the surprisal reading. The corrected paragraph follows; the original is struck through rather than deleted.

~~**Mode 1 — independent windows.** No retrieval signal appears beyond very short range: `Δlnp` is positive only for `d≤32` and negative or flat from 32 to 512 bytes — seeing an entity earlier in the same window does not make the model more confident about it later.~~

**Mode 1 — independent windows (as trained), distances capped at `seq=512`. Corrected reading: retrieval signal is present, and it lives in the 32–256 byte range.** Against the same-entity `nowin` reference (a prior occurrence exists but fell outside the window, so the model cannot see it — the honest no-memory baseline for this architecture as trained), for `T=1`:

| bucket | n | top1% | rank | mean_lnp (surprisal) | Δlnp | vs. no-memory |
|---|---|---|---|---|---|---|
| d ≤ 32 | 12 | 8.33 | 13.8 | 3.694 | +0.661 | **worse** |
| 32 < d ≤ 128 | 116 | **50.00** | **6.9** | **2.204** | **−0.829** | **much better** |
| 128 < d ≤ 256 | 182 | 39.01 | 8.9 | 2.707 | −0.326 | better |
| 256 < d ≤ 512 | 91 | 35.16 | 10.4 | 3.015 | −0.018 | ≈ same |
| `nowin` (no memory) | 1039 | 34.94 | 9.8 | 3.033 | — | reference |

All three quantities agree: at 32–128 bytes the model is right 50.00% of the time against a 34.94% no-memory baseline, ranks the correct byte 6.9 against 9.8, and is 0.83 nats less surprised. **In-context memory measurably helps on real text between roughly 32 and 256 bytes, and its benefit is gone by 512.**

The one bucket where memory *hurts* is `d≤32` (top1 8.33% vs 34.94%), and it carries two caveats that make it the least trustworthy row: `n=12`, and the populations are not matched — a `nowin` occurrence is typically a scene re-entry, predictable from immediate context, whereas a `d≤32` occurrence is a within-scene repetition. That asymmetry was already flagged when the benchmark was built and remains unresolved.

What also helps, separately, is having seen the entity anywhere during training (`cold`, first-ever occurrence: surprisal ≈ 4.1–4.5) versus a window where it recurs but out of reach (`nowin`: ≈ 3.0–3.1) — that gap is lexical knowledge of the book, not in-context retrieval. `T=1` and `T=1.3` remain statistically indistinguishable on this metric, so the *temperature fix* is still null here; what is not null is ClockMem's in-context memory itself.
...
> **CORRECTION (2026-08-25): the measurements in this section stand; the explanation offered for them does not.** Wherever this section attributes the 8-key collapse to "a partition of 512 channels running out of room" — including the phrases *"what makes the channel-partitioning account hard to escape"*, *"the signature predicted by the channel-partitioning account"*, and *"where a channel-partitioned representation runs out of room"* — that account was **refuted** by the width sweep, the query-swap diagnostic, the data-order lottery, and the channel ablation. Two configurations with identical channels-per-key differ by 21 points; effective capacity is ~5 channels in every model, winner or blind; and the element-wise write demonstrably *can* bind, at 99.77%, in 2 runs out of 17. The failure is one of **optimization**, not representation or capacity. **Section 4.12 supersedes every causal claim below**; the numbers, the controls and the failure-mode taxonomy remain valid. Kept as written because what was believed, and on what evidence, is part of the record.
...
Section 4.11 attributed the 8-key collapse to a partition of channels running out of room. **That account is refuted.** Four independent results, in the order they arrived:

**(a) Channels-per-key does not predict anything.** A width sweep decouples `dim` from key count for the first time (every earlier run varied only the number of keys):

| config | channels/key | accuracy |
|---|---|---|
| dim512 / k2 | 256 | 99.65% |
| dim256 / k4 | 64 | 48.56% |
| dim512 / k8 | 64 | 23.42% |
...
Two cells with the *same* 128 channels/key differ by 21 points, and `dim=512` produces both the best and one of the worst results. A counting argument makes the same point: storing which of 32 values a key takes is **5 bits**; 64 channels is an enormous surplus. **Capacity was never the binding constraint.**

**(b) Several models never read the query at all.** A diagnostic (`examples/query_swap.rs`) re-runs an identical window changing only *which already-bound key is being asked about*; everything prior is bit-identical. If the model binds, `P(val_A)` collapses and `P(val_B)` rises. A control perturbs a *filler* byte instead, calibrating how much that model's output moves under an irrelevant change.

| config | accuracy | directional Δ | ratio vs. filler noise |
|---|---|---|---|
| dim512 / k2 | 99.65% | +0.9941 | 697× |
| dim256 / k4 | 48.56% | +0.2157 | 23× |
| dim512 / k8 | 23.42% | +0.0341 | 6.65× |
| dim512 / k4 | 28.82% | −0.0005 | 0.50× |
| dim256 / k2 | 50.38% | +0.0001 | 0.28× |

Sorted by query-reading, the accuracies fall out on their own — which neither width nor key count predicts. The bottom two move their output *less* when asked about a different key than when a garbage byte is flipped: they are not partially binding, they are ignoring the question. Their 28.82% and 50.38% are exactly 1/4 and 1/2 of the candidates — coins, not weak links.

*Methodological note:* an early version of this diagnostic reported total distributional movement rather than directional movement, and produced a reproducible 1.45× ratio on a checkpoint whose actual directional shift was +0.0005 — it moved, and pointed nowhere. Swapping one key byte for another perturbs the distribution for reasons unrelated to identity. `P(val_A | ask A) − P(val_A | ask B)` is the only form that separates binding from movement.

**(c) The outcome is a lottery over data order, at ~12%.** Same config (`dim=256`, 2 keys, 22,499 windows, 1 epoch), **identical initial weights** (see the `--seed` correction in Section 2), same corpus, same binary. The only difference is the order in which the same windows are visited:

| run | val bpb | reads the query | accuracy |
|---|---|---|---|
| seed 7 | 7.303 | no (+0.000) | ~50% |
| seed 8 | 7.345 | no (+0.000) | 41.81% |
| seed 9 | 7.298 | no (+0.000) | 43.91% |
| **seed 10** | **7.235** | **yes (+0.985)** | **99.77%** |
| seed 11 | 7.309 | no (−0.001) | 45.75% |

Extended to **2 of 17 (11.8%**, CI 1.5–36%). Three consequences: the objective *does* reward binding (the winner has the best bpb by 0.063 with no overlap, so **val bpb alone identifies the winner** — a free filter from the log); the outcome is decided in the first quarter of training (at step 5000 the winner is already at +0.197 while the blind runs sit at +0.0002, and in the blind runs the behaviour never appears even transiently); and, decisively, **the element-wise write CAN bind, at 99.77%.** It almost never finds it. **The problem is optimization, not representation** — which invalidates the motivation for the low-rank write as originally stated.

**(d) The binding lives in 4 channels out of 256, and the blind models use the same ones.** Ablating the *read* of specific channels (`EVA_READ_MASK`, setting `g[c]=0` without touching the write or the state dynamics) on the winner:

| channels masked | directional Δ |
|---|---|
| none | +0.8849 |
| 8 from the fast end (248–255) | +0.8755 (intact) |
| 8 from the middle (124–131) | +0.8894 (intact) |
| 8 scattered at random | +0.8879 (intact) |
| **0–3** | **+0.0004 (destroyed)** |
| channel 0 alone | +0.2230 (−75%) |

Perfect specificity: any eight channels elsewhere do nothing; four specific ones destroy the capability. This connects directly to Section 3 — the learned spectrum is 251 fast (98%) / 4 medium / 1 very slow, and a binding must survive a ~45-byte gap, which the 251 fast channels physically cannot. Five channels remain, and the ablation points at exactly those.

The decisive part: **the blind models have the same spectrum (251/4/1, matching to the percentile) and use the same channels** — masking 0–3 takes the blind model from 43.59% to 3.85% (chance). Same substrate, same four channels, two different circuits written on top: the blind run stores *"these are the values in play"*, the winner stores *"key A goes with value X"*. This kills any remaining capacity account: effective capacity is ~5 channels in **every** model, which is why width never predicted anything.

**Two further results from the same round.** The output gate `g` is **not** the culprit: `EVA_NO_GATE=1` over 12 matched pairs gave **0 of 12** against a base rate of 2 of 17 (criterion fixed in advance at ≥5 of 12; Fisher p≈0.34). This does not contradict Section 4 — there the read existed and the gate suppressed it, here the read never forms and removing the gate does not help it form. And a practical one: **the final checkpoint is worse than an earlier one.** The winner scored 99.77% (directional +0.9852) at step 20,000 and 92.18% (+0.8868) at the end, 249 steps later, for 0.006 bpb — the binding metric is far more sensitive than bpb because queries are a small fraction of bytes. Every number in this draft is from a last-step checkpoint and may therefore understate the peak (n=1, small magnitude).

**Still unanswered:** whether the lottery is decided by the data ordering or by the initial weights. That axis was untestable until `EVA_INIT_SEED` existed (Section 2 correction), and the first attempt to measure it — 12 runs with `--seed` fixed and initialization varying — is **void**: a wrapper's guard against copying a partially-written checkpoint caused it to copy the *step-2500* file instead of the step-5000 one in all twelve runs (detected because the twelve results were identical to four decimals; `md5sum` confirmed the same file). Calibration afterwards showed the winner is indistinguishable from the blind runs at step 2500 (+0.0001 vs. +0.1979 at step 5000). Roughly ten machine-hours were lost and the initialization axis remains open.

### 4.13 Does any of this transfer? Eight seeds on real text say the lottery is a property of the toy

Every number above is from a synthetic benchmark in which ~10% of positions are queries over 2–8 keys. Before spending further compute on the mechanism, the prior question was whether the phenomenon exists outside it. Two facts made this urgent: the real-text null of Section 4.8 was **n=1**, and a methodology that produces seven blind runs out of eight makes any single null a possible losing ticket; and every run in the project's history had used `--seed 7`, so with a constant initialization **the same configuration had never been run twice on real text.** There was no dispersion to inspect — it had to be generated.

**Eight seeds on the full *Don Quijote*** (2.2MB, `dim=512, ffn=1024, 5 blocks, seq=512`, 1 epoch, the recipe of Section 4.7). Seed 7 ran first as a control and reproduced the paper's 1.716 exactly.

| seed | 7 | 26 | 25 | 27 | 21 | 24 | 22 | 23 |
|---|---|---|---|---|---|---|---|---|
| val bpb | 1.716 | 1.717 | 1.719 | 1.721 | 1.722 | 1.724 | 1.726 | 1.743 |

Mean 1.7235, range 0.027, unimodal with one tail. **But global bpb cannot see this phenomenon** — the synthetic runs measured that 0.006 bpb corresponds to 7.6 points of binding accuracy, because queries are a small fraction of bytes, and in 2.2MB of prose that fraction is far smaller still. The instrument was chosen for being cheap, not for being sensitive. So the directed analogue was run on all eight checkpoints — per-seed entity retrieval, Δlnp against the no-memory reference (negative = memory helped):

| seed | d≤32 | 32–128 | 128–256 | 256–512 |
|---|---|---|---|---|
| 7 | +0.661 | −0.829 | −0.326 | −0.018 |
| 21 | +0.638 | −0.702 | −0.101 | +0.014 |
| 22 | +0.812 | −0.490 | −0.057 | +0.126 |
| 23 | +0.868 | −0.833 | −0.180 | −0.128 |
| 24 | +0.642 | −0.625 | −0.110 | −0.050 |
| 25 | +0.384 | −0.740 | −0.279 | −0.080 |
| 26 | +0.636 | −0.585 | −0.160 | −0.045 |
| 27 | +0.647 | −0.779 | −0.134 | −0.029 |

In the highest-signal bucket (32–128): mean **−0.698**, sd **0.123**, all eight within **1.4 sd**. Continuous variation, no separated group. The contrast with the synthetic benchmark is stark: there the winner sat **~13 sd** from the blind cluster (+0.985 against +0.000, sd ≈0.0005) — categorical. Here there is nothing of the sort, and two instruments of very different sensitivity (global bpb and entity retrieval) agree.

**Conclusion: the lottery is a property of the toy.** The mechanism — the four channels, the query-reading, the 2-in-17 — is real and well measured, but on a synthetic task with a discrete key/value structure that offers a basin to find or miss. Real prose has no such structure, and no such lottery appears.
...
- **The proposed outer-product write is not a new idea, and should not be framed as one.** `k vᵀ` accumulated into a matrix state, read by a query, *is* linear attention — the family this draft already places ClockMem in (Section 2). The honest question it poses is not novelty but this: what happens when ClockMem is given an explicit associative representation while holding state size fixed and cost low. Section 4.12 further removes the original motivation for building it, since the element-wise write turns out to be capable of binding.
- **The optimization-lottery result (Section 4.12c) has an obvious neighbourhood in the literature we have not surveyed**: seed-variance and "lottery ticket"-style findings, and the induction-head formation literature already cited, all concern whether a circuit forms rather than whether it can be represented. We have not done that survey, and the 11.8% figure should not be presented as novel until we have.

What we believe is not simply restated from prior work: the two-mechanism causal chain (parametrization trap in the decay, independently a second parametrization trap in the output gate, in the *same* trained model, each diagnosed by direct intervention rather than inferred); the empirical demonstration that a structurally-restricted hypothesis class (R1c) succeeds where an unrestricted one with strictly more expressive power (R1a/R1b) fails on the identical task; and the localization of an entire binding capability to four channels shared, at the same spectrum, by models that succeed and models that fail (Section 4.12d). Given the precedent above, the honest framing of the open question is narrower than "does long-range recurrent memory work" — it is closer to: *can a recurrent memory achieve retention, positional resolution, and learnable gating simultaneously, without the gating route reintroducing the same class of trap that afflicts the decay parametrization on its own?* A direct head-to-head against a modern SSM baseline (same delayed-copy task, matched N sweep, comparing bpb, FLOPs, state size, and positional-recovery accuracy) is the natural next step before any claim of contribution is finalized — not yet run.
...
- **Two reported results in this draft were wrong in ways that survived review for days, and both were sign/definition errors rather than measurement errors.** Section 4.8 inverted a surprisal, converting a positive real-text result into a reported null; Section 4.7 compared two incompatible band definitions and reported a corpus-dependent spectrum that does not exist. Both are corrected in place with the original text retained. The general lesson is recorded here rather than buried: **every derived quantity in this work should be cross-checked against at least one quantity that does not share its sign convention** — `top1%` and `rank` settled the first error immediately once anyone thought to look.
- **The reverse-engineering of the winning circuit (Section 4.12d) is n=1.** The ablations are deterministic and the controls are clean, but the circuit is characterized on a single model; whether the concentration into four channels reproduces in the other winner (`dim512/k2`) is untested. The channel-swap experiment that would directly confirm that key identity lives in that subspace requires manipulating the state rather than the read gate, and was not run.
- **The initialization axis is still open**, and its first measurement is void (a wrapper bug copied step-2500 checkpoints while reporting step-5000; Section 4.12). Whether the lottery is decided by the data ordering or by the starting weights is therefore unknown.
- **The gate-ablation counts (2/17 vs 0/12) could not be re-verified from raw logs** at the time of writing: the per-checkpoint diagnostic outputs were not retained on the second machine. Flagged as a pending audit.
- **All reported numbers come from last-step checkpoints, and the last step is not the best one** (Section 4.12): on the one case measured, the binding metric fell 7.6 points over the final 249 steps for 0.006 bpb. Reported figures may systematically understate peaks.
...
- ~~An 8-key convergence run to rule out undertraining, and a matched 8-key attention run.~~ **Both done** (Section 4.11); and the account they were meant to test — channel partitioning — was subsequently **refuted** by Section 4.12, so their interpretation there has been superseded.
- ~~Conditional on the channel-partitioning account surviving, implement a low-rank outer-product write (`k vᵀ`).~~ **The account did not survive, and the motivation as originally stated is withdrawn** (Section 4.12c: the element-wise write binds at 99.77% when the lottery goes its way). The op is nonetheless **implemented and verified but never measured**: `ops::clockmem_lowrank_from` behind `EVA_WRITE_LOWRANK=r`, with gradchecks on all 11 tensors, mutation-testing of the backward, and a zero-init projection so an A/B starts bit-identical to the baseline. If it is measured, three things now constrain how: (i) the honest framing is not novelty — `k vᵀ` with matrix state *is* linear attention, a family this draft already places ClockMem in — but "what happens when ClockMem is given an explicit associative representation at fixed state size and low cost"; (ii) with a base rate of 11.8%, one run per arm measures the lottery, not the architecture, so it needs several seeds per arm; and (iii) it must be measured where the model *does* read the query (e.g. `dim256/k4`), since in a blind configuration it would produce an uninterpretable null. `query_swap` therefore becomes an **admission criterion**, not a diagnostic.
- **The initialization axis** (Section 4.12, void first attempt): `EVA_INIT_SEED` varying weights with `--seed` fixed, ~12 short runs. This is the exact complement of the data-order result and the only structural axis never tested.
- **Save the best checkpoint, not the last** (Section 4.12): with the binding metric an order of magnitude more sensitive than bpb, the current convention costs measured results on every run.
...
- **The one live real-text lead** (Section 4.13): in-context memory helps between 32 and 256 bytes across all eight seeds, and switches off past 256. Why it switches off there — and whether the Section 3 saturation trap is what sets that ceiling — is the most direct question this draft's own positive result raises, and it is untouched.
