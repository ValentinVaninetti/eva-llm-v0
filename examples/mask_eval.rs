//! Masked eval for the long-range synthetic benchmark (lead decision
//! 2026-08-14).
//!
//! The metric that decides the thread: CONDITIONAL bpb over the recurrent
//! positions t >= N of each window. Without the mask, the seed floor (N
//! bytes per window, unpredictable for any model) would contaminate the
//! N -> error curve and make N=256 look harder than it is even for a model
//! with perfect memory.
//!
//! Recurrent region of window i (same rule as the generator): absolute
//! positions [i*seq+N, (i+1)*seq), or relative targets t in [N-1, seq-2].
//! That region is predictable with perfect memory (bpb 0) and uniform
//! without memory (bpb 8). The last target of each window (t=seq-1, the
//! first byte of the next window) is excluded due to the boundary.
//!
//! USAGE: cargo run --release --example mask_eval -- <weights> <data> <N>
//!   weights trained checkpoint (with --seq 512, --val 0.1)
//!   data    corpus generated by generate_synthetic
//!   N       dependency distance (the same one used by the generator)
//!
//! The val cut is the same contiguous one train uses (last 10%).
