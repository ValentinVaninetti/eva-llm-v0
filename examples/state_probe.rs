...
//! windowed readout could recover) linearly contain the value the model
//! needs to predict target[t]?
//!
//!   target[t] = F(input[t-N+1])        (the generator's recurrence)
//!   cur[t]    = beta * sum_j alpha^j * k[t-j]*v[t-j]   (leaky state)
//!   w[tau,c]  = (cur[tau,c] - alpha_c*cur[tau-1,c])/beta   = the WRITE at
//!               position tau, recovered by the finite difference that a
//!               read window of size >= N+1 CAN express.
//!
//! We fit RIDGE probes (features z-scored on train) and report val bpb:
//!
//!   Probe A (single state):   cur[t]             -> input[t-N+1]
//!   Probe B (windowed read):  w[t-N+1]           -> input[t-N+1]
//!
//! Pre-registered disambiguation:
//!   A fails AND B fails  -> STORAGE: the value never made it into the state
//!   A fails AND B works  -> READ:    the value is there but out=q*cur*g
//!                           can't extract a specific past position
//!   A works              -> even the current read has the value in reach
//!
//! Implementation notes:
//! - The clock state lives inside ops::clockmem, so this example REPLICATES
//!   the block forward exactly (same ops, same order, same alpha squash).
//!   State is FRESH per window (the same convention as mask_eval), which
//!   makes windows independent and lets us sample train windows instead of
//!   forwarding the whole corpus.
//! - It reads the same EVA_ALPHA_TEMP / EVA_ALPHA_ANTISAT envs as the real
//!   forward.
//!
//! USAGE: cargo run --release --example state_probe -- <weights> <data> <N> [train_windows]
//!   weights           checkpoint trained on this corpus (val cut = last 10%)
//!   data              corpus generated by generate_synthetic
//!   N                 dependency distance
//!   train_windows     optional, default 200: windows used to fit the probe
