...
impl EvaBlock {
    pub fn new(d: usize, ffn: usize, kernel: usize, eps: f32, rng: &mut Rng) -> Self {
        EvaBlock {
            norm0: RMSNorm::new(d, eps),
            conv: DepthwiseConv1d::new(d, kernel, rng),
            norm1: RMSNorm::new(d, eps),
            clock: ClockMem::new(d, rng),
...
impl Module for EvaBlock {
    fn forward(&self, x: &Tensor) -> Tensor {
        ...
        let h = ops::add(&h, &self.clock.forward(&self.norm1.forward(&h)));
    ...
    fn parameters(&self) -> Vec<&Tensor> {
        ...
        out.extend(self.clock.parameters());
    ...
    fn parameters_mut(&mut self) -> Vec<&mut Tensor> {
        ...
        out.extend(self.clock.parameters_mut());
    ...
    fn named_parameters(&self, prefix: &str) -> Vec<(String, &Tensor)> {
        ...
        out.extend(self.clock.named_parameters(&format!("{}.clock", prefix)));
    ...
    fn named_parameters_mut(&mut self, prefix: &str) -> Vec<(String, &mut Tensor)> {
        ...
        out.extend(self.clock.named_parameters_mut(&format!("{}.clock", prefix)));
